...
gnome-terminal --title="MASTER-HFD" -- bash -c "ros2 launch hfd_teleoperation master_hfd.launch.py; exec bash" & terminal_pids+=($!)

# Launch slave_realman node
echo "Launch slave_realman node..."
gnome-terminal --title="SLAVE-REALMAN" -- bash -c "ros2 launch realman_teleoperation slave_realman.launch.py; exec bash" & terminal_pids+=($!)

# Launch slave_ctek node
echo "Launch slave_ctek node..."
gnome-terminal --title="SLAVE-CTEK" -- bash -c "ros2 launch ctek_teleoperation slave_ctek.launch.py; exec bash" & terminal_pids+=($!)

# Launch slave_realsense node
echo "Launch slave_realsense node..."
gnome-terminal --title="SLAVE-REALSENSE" -- bash -c "ros2 launch realsense_teleoperation slave_realsense.launch.py; exec bash" & terminal_pids+=($!)
